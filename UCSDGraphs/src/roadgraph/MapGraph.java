...
public class MapGraph {
	...
	public void addEdge(GeographicPoint from, GeographicPoint to, String roadName,
		...
		//TODO: Implement this method in WEEK 3
		if(!nodes.containsKey(from) || !nodes.containsKey(to) || length < 0 || from == null || to == null || roadName == null || roadType == null) {
			 					     ...
			 					     GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		while ( !queue.isEmpty() ) {
			...
			for ( MapEdge edge : nodes.get(curr).getEdges()) {
				GeographicPoint neighbor = edge.to;
	...
	private List<GeographicPoint> reconstructPath(Map<GeographicPoint, GeographicPoint> parent, GeographicPoint start, GeographicPoint goal) {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		for ( MapNode node : nodes.values() ) {
			...
			node.estimatedDistance = Double.POSITIVE_INFINITY;
		}
		
		MapNode startNode = nodes.get(start);
		MapNode goalNode = nodes.get(goal);
		PriorityQueue<MapNode> pq = new PriorityQueue<>(Comparator.comparingDouble(n -> n.distance));
		...
		while( !pq.isEmpty() ) {
			...
			if( visited.contains(curr.getLocation())) continue;
			...
			for( MapEdge edge : curr.getEdges() ) {
				MapNode neighbor = nodes.get(edge.to);
				if( !visited.contains(neighbor.getLocation()) ) {
					double newDist = curr.distance + edge.length;
					if( newDist < neighbor.distance ) {
						neighbor.distance = newDist;
						parentMap.put(neighbor.getLocation(), curr.getLocation());
						pq.add(neighbor);
					}
				}
			}
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
	    ...
	    startNode.distance = 0;
	    startNode.estimatedDistance = start.distance(goal);
	    ...
	    while ( !pq.isEmpty() ) {
	        ...
	        for ( MapEdge edge : curr.getEdges() ) {
	            ...
	            if ( !visited.contains(neighbor.getLocation()) ) {
	                ...
	                if ( newDist < neighbor.distance ) {
	                    neighbor.distance = newDist;
	                    neighbor.estimatedDistance = newDist + neighbor.getLocation().distance(goal);
	                    parentMap.put(neighbor.getLocation(), curr.getLocation());
	                    pq.add(neighbor);
	                }
	            }
