...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		List<Integer> firstNeighbor = this.getNeighbors(v);
		 List<Integer> secondNeighbor = new ArrayList<Integer>();
		 List<Integer> distance2 = new ArrayList<Integer>();
		 for ( int i = 0; i < firstNeighbor.size(); i++ ) {
			 secondNeighbor = getNeighbors(firstNeighbor.get(i));
			 for ( int j = 0; j < secondNeighbor.size(); j++ ) {
				 distance2.add(secondNeighbor.get(j));
			 }
		 }
		 return distance2;
	}
